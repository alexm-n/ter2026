...
# =====================================================================
# 2. DATASETS (Modifiés pour accepter une liste précise de patients)
# =====================================================================
...
# =====================================================================
# 3. FONCTIONS D'ENTRAÎNEMENT AVEC VALIDATION
# =====================================================================

def train_simclr(model, train_loader, val_loader, device, epochs, save_path):
    print(f"\n--- PRÉ-ENTRAÎNEMENT SIMCLR ({epochs} Époques) ---")
...
def train_segmentation(seg_model, train_loader, val_loader, device, epochs, save_path):
    print(f"\n--- FINE-TUNING SEGMENTATION ({epochs} Époques) ---")
...
# =====================================================================
# 4. EXÉCUTION PRINCIPALE (Le Panneau de Contrôle)
# =====================================================================
...
def main():
    ...
    # Calcul dynamique des époques
    facteur = 100.0 / POURCENTAGE_A_UTILISER
    simclr_epochs = int(BASE_EPOCHS_SIMCLR * facteur)
    seg_epochs = int(BASE_EPOCHS_SEG * facteur)

    print("==================================================")
    print(f" DÉMARRAGE DE L'EXPÉRIENCE À {POURCENTAGE_A_UTILISER}% DU DATASET")
    print(f" -> L'encodeur SimCLR va tourner sur {simclr_epochs} époques")
    print(f" -> Le décodeur Seg va tourner sur {seg_epochs} époques")
    print("==================================================\n")
    ...
    # Récupération et split déterministe (Seed = 42)
    ...
    patients_train_subset = tous_les_patients_train[:max(1, nb_patients_voulus)] # max(1) empêche d'avoir 0
    patients_val = sorted(glob.glob(os.path.join(val_dir, "BraTS2021_*")))

    print(f"Patients pour l'Entraînement : {len(patients_train_subset)}")
    print(f"Patients pour la Validation  : {len(patients_val)}\n")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # Noms des fichiers dynamiques
    ...
    if os.path.exists(simclr_weights_path):
        print(f"Poids trouvés pour SimCLR {POURCENTAGE_A_UTILISER}% ! Chargement direct...")
    ...
    # --- SAUVEGARDE DU TABLEAU DE BORD (CSV) ---
    if len(historique_complet) > 0:
        df = pd.DataFrame(historique_complet)
        df.to_csv(csv_history_path, index=False, sep=";")
        print(f"\n✅ L'historique complet des pertes (Train vs Val) a été sauvegardé dans : {csv_history_path}")
